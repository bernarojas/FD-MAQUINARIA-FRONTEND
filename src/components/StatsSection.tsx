export default function StatsSection() {
  ...
  return (
    <section id="nosotros" className="py-16 bg-slate-900/60 border-y border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10">
          {stats.map((stat) => (
            <div key={stat.label} className="text-center">
              <div className="text-3xl md:text-4xl font-black text-amber-500">{stat.value}</div>
              <div className="mt-2 text-slate-400 text-xs font-medium leading-tight">{stat.label}</div>
            </div>
          ))}
        </div>
        <div className="mt-12 pt-10 border-t border-slate-800 max-w-3xl mx-auto text-center">
