  ...
  return (
    <div className="space-y-3">
        ...
        <div key={i} className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2.5 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-colors text-sm"
            />
          ...
          <div>
            ...
            {doc.url ? (
              <div className="flex items-center gap-3">
                ...
                <label className="text-xs text-slate-400 hover:text-slate-600 cursor-pointer transition-colors">
                  Cambiar
                  <input type="file" accept="application/pdf" onChange={(e) => handleFile(i, e)} className="hidden" />
                </label>
              </div>
            ) : (
              <label className={`flex items-center gap-2 border border-dashed rounded-lg px-3 py-2.5 cursor-pointer transition-colors ${doc.uploading ? 'border-blue-400 text-blue-600' : 'border-slate-300 hover:border-blue-400 text-slate-400 hover:text-slate-600'}`}>
      ...
      <button
        type="button"
        onClick={addDoc}
        className="w-full border border-dashed border-slate-300 hover:border-blue-400 text-slate-400 hover:text-blue-600 py-3 rounded-xl text-sm font-medium transition-colors"
      >
        + Agregar documento
      </button>
    </div>
